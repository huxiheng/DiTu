...
//                    [LSDialog showAlertWithTitle:@"提示" message:@"已经是最新版了" callBack:0];
    ...
    if (indexPath.row == 3) {
        XSRemoveCountBindView *unbindView = [[XSRemoveCountBindView alloc] initWithIphoneNum:[LoginUtil numberPhone] countName:[LoginUtil loginUserName]];
        dpBlockSelf;
...
- (void)dataQequestForUnbind{
    ...
    NSString *phoneNumber = (([LoginUtil numberPhone] == NULL) ? @"":[LoginUtil numberPhone]);
